...
import { Shield, User, Mail, Lock, ArrowRight, Github, Chrome, Loader2, AlertCircle, CheckCircle2, WifiOff, Cloud, Eye, EyeOff } from "lucide-react";
...
export default function Register({ onRegister, onToggleMode }: RegisterProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authError) {
        siemLogger.logAuth(email, 'register', false);
        setError(authError.message);
      } else {
        siemLogger.logAuth(email, 'register', true);
        onRegister(); // Immediate Transition
      }
    } catch (err) {
  ...
  const handleSocialLogin = async (provider: 'google' | 'github') => {
    ...
    try {
      ...
      if (timeoutId) clearTimeout(timeoutId);
      setError("Cloud Identity Tunnel closed. Check your configurations.");
      setIsSocialLoading(null);
    }
  };
  ...
  const handleMockApprove = () => {
    ...
    setTimeout(() => {
      if (provider) siemLogger.logAuth(provider, 'register', true);
      onRegister();
      setIsSocialLoading(null);
    }, 1000);
  };
        ...
        <div className="glass-card p-8 md:p-10 relative overflow-hidden">
          ...
          <div className="flex flex-col items-center mb-10">
            ...
            <div className="flex items-center gap-2 mb-6 font-label">
              ...
              <p className="text-[10px] text-text-muted uppercase tracking-[0.3em] font-black">Operator Provisioning</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2 font-label">
              <label className="text-[9px] text-text-muted uppercase font-black tracking-[0.2em] ml-2">Operator Name</label>
              <div className="relative group">
                <User className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-text-muted group-focus-within:text-accent-blue transition-colors duration-300" />
                <input 
                  type="text" 
                  required
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder="Field Operator Alpha"
            ...
            <div className="space-y-2 font-label">
              <label className="text-[9px] text-text-muted uppercase font-black tracking-[0.2em] ml-2">Identity (Email)</label>
              <div className="relative group">
            ...
            <div className="space-y-2 font-label">
              <label className="text-[9px] text-text-muted uppercase font-black tracking-[0.2em] ml-2">Secure Key (Password)</label>
              <div className="relative group">
            ...
            <button 
              ...
              className="btn-primary w-full py-4 flex items-center justify-center gap-3 group disabled:opacity-50 transition-all mt-4"
            >
              <span className="relative z-10 flex items-center gap-2 font-bold uppercase tracking-widest text-xs text-white">
                {isEmailLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Request Access"}
